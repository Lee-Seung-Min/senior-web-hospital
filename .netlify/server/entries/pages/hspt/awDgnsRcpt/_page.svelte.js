...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<section id="con"><div class="content"><div class="form_table"><h2 class="page_title">진료 접수/예약</h2>
      <div class="box_search" style="display: flex; align-items: center; gap: 10px;"><span>환자명</span>
        <span><label><input type="text" name="" id="" placeholder="검색어를 입력하세요"${add_attribute("value", searchText, 0)}></label></span>
        <span>생년월일</span>
      ...
      ${validate_component(Gridjs, "Grid").$$render(
      $$result,
      {
        class: "gridjs",
        columns,
    ...
    <div class="form_table"><h3 style="margin-bottom: 10px;">회원 정보 접수/예약</h3>
      ...
      return `<option${add_attribute("value", itemList.itemId, 0)}>${escape(itemList.itemName)}</option>`;
        ...
        <tr><th>진료 사유</th>
          <td colspan="3"><div class="input_box_no" style="height:120px;"><textarea placeholder="진료 사유 작성">${escape("")}</textarea></div></td></tr>
        <tr></tr></table></div>
    <div style="display: flex; justify-content: flex-end; margin-top:10px;"><button type="button" class="mbtn_b" style="margin-top:10px; align-items: center !important;" ${!selectedMbrData.patName || !selectedDgnsType || !selectedHour || !selectedDoctor || !selectedItemId || !selectedDeptId ? "disabled" : ""}>접수/예약
      </button></div>
    ${validate_component(Footer, "Footer").$$render($$result, {}, {}, {})}

    ${``}

    ${`${validate_component(PageLoader, "PageLoader").$$render($$result, {}, {}, {})}`}</div>
