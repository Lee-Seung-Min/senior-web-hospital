<script>
  ...
  async function fetchData(searchText = "", birthDate = null) {
    isLoading = true;
    try {
      let url = `${authUrlAddr}/v1/member/selectMbrList`;
      const params = [];
      if (searchText)
        params.push(`dgnsPatName=${encodeURIComponent(searchText)}`);
      if (birthDate) params.push(`birthDate=${birthDate}`);
...
<section id="con">
  <div class="content">
    <div class="form_table">
      <h2 class="page_title">진료 접수/예약</h2>
      <div
        class="box_search"
      ...
      >
        ...
        <button
          type="button"
          class="mbtn_n_5"
          on:click={() => fetchData(searchText, formatDateTime(birthDate))}
          >검색</button
        >
      </div>
      <Grid
    ...
    {#if rcptSuccessPopup}
      <section class="z_alert">
        <div class="z_box_1">
          ...
          <div class="btn_wrap">
            <button class="btn_02" on:click={() => (rcptSuccessPopup = false)}
              >확인</button
            >
          </div>
        </div>
