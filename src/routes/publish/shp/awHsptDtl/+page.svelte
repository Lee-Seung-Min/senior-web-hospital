<script>
  ...
  import doctorM from "$lib/img/barodoctor/doctor_0.png";
  import doctorF from "$lib/img/barodoctor/doctor_6.png";
  ...
  let hdayList = [];
  let medicalItemList = []; // 선택된 itemId들만 모아둘 배열
  let gridKey = 0;
  ...
  async function getDoctorInfo(e) {
    ...
    if (fileInput) fileInput.value = "";   // 또는: fileInput?.value = "";
  ...
  function getImg(imgUrl) {
    if (imgUrl != null) {
      ...
      imgSource = (docInfo.dtrGender === "F") ? doctorF : doctorM;
      selectedFile = "";
      return imgSource;
    }
  }
  ...
  //의사 진료항목 체크
  async function getDtrItemChecked() {
  const selected = dtrItemLst.filter((item) => item.ditmSelYon);
  docInfo.dtrItems = selected.map((item) => item.itemName).join(", ");
  dtrItemIdList = selected.map((item) => item.itemId);
  ...
  async function getHldayList() {
    ...
    holInfoList = Array.isArray(resData.resultVO) ? resData.resultVO : [];
  }
          ...
          >
            <a
              ...
              on:click={async () => {
                ...
                activeTab = 1;
                 await tick();
                gridKey += 1; // remount로 컨테이너/그리드 상태 리셋
              }}
              >의사정보
            </a>
          </li>
      ...
      >
        <div class="form_table list">
          ...
          <div>
            <div class="box_1">
              <div class="form_table">
                {#if browser && (whatForm === "doctorInfo" || whatForm === "doctorTimeInfo")}
                  {#key gridKey}
                    <Grid
                      data={docData}
                      columns={dtrColumns}
          ...
          <table class={dtrType === "dtrLoad" ? "doctorSelect" : ""}>
            ...
            <tr>
              ...
              <td colspan="3">
                <img
                  src={imgSource || (docInfo.dtrGender === "F" ? doctorF : doctorM)}
                  class="doctorImg"
                  alt="."
                />
                <div class="flex_warp">
      ...
      <div class="form_table">
        <table class="holiday_tbl">
          ...
          <tr>
            <th>안내문구 :</th>
            <td>
              <textarea class="desc" style="width:100%;" bind:value={$shdyDesc}></textarea><!---->
            </td>
          </tr>
        </table>
      </div>
  ...
  <section class="z_alert">
    ...
    <div class="z_box_1">
      ...
      <div class="btn_wrap">
        ...
        >
        <button class="btn-lg gray" on:click={() => xButton()}>취소</button>
      </div>
    </div>
  </section>
{/if}
...
<style>
  /* Remove horizontal padding from content to maximize tab_container width */
  #content {
    padding-left: 0 !important;
    padding-right: 0 !important;
  }
  
  /* Make tab_container and its children use full width */
  .tab_container {
    width: 100%;
    padding: 0;
  }
  
  /* Ensure tab content spans full width */
  .tab_content {
    width: 100%;
  }
  
  /* Make form_table inside tabs use full width with minimal padding */
  ...
  /* Ensure tables inside stretch to container width */
  .tab_content .form_table table {
    width: 100%;
  }

  /* Change active tab button color to red for time tabs (월~일) */
  .tabs_pill li.active a {
    background-color: #dc3545 !important;
    border-color: #dc3545 !important;
    color: white !important;
  }

  /* Hover effect for time tab buttons */
  .tabs_pill li a:hover {
    background-color: #c82333 !important;
    border-color: #bd2130 !important;
    color: white !important;
  }

  /* Change blue buttons (btn_05, btn_02) to red */
  .btn_05 {
    background-color: #dc3545 !important;
    border-color: #dc3545 !important;
    color: white !important;
  ...
  /* For modern browsers with accent-color support */
  input[type="checkbox"] {
    accent-color: #dc3545;
  }

  /* Make Grid container responsive to content height in tab3 */
  #tab3 .form_table.list {
  ...
  #tab3 .box_1 .form_table {
    min-height: auto !important;
    height: auto !important;
    padding: 0;
  }

  /* Ensure Grid wrapper adapts to content */
  #tab3 .gridjs-wrapper {
    height: auto !important;
    min-height: auto !important;
  }

  #tab3 .gridjs-container {
    height: auto !important;
  }
  ...
  /* Ensure buttons in tab3 are clickable */
  #tab3 .btn_wrap {
    position: relative;
    z-index: 10;
    margin-bottom: 30px;
